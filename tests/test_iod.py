...
def test_vallado75(lambert):
    k = Earth.k
    r0 = [15945.34, 0.0, 0.0] * u.au
    r = [12214.83399, 10249.46731, 0.0] * u.au
    tof = 76.0 * u.min

    expected_va = [2.058925, 2.915956, 0.0] * u.au / u.s
    expected_vb = [-3.451569, 0.910301, 0.0] * u.au / u.s
...
def test_curtis52(lambert):
    k = Earth.k
    r0 = [5000.0, 10000.0, 2100.0] * u.au
    r = [-14600.0, 2500.0, 7000.0] * u.au
    tof = 1.0 * u.h

    expected_va = [-5.9925, 1.9254, 3.2456] * u.au / u.s
    expected_vb = [-3.3125, -4.1966, -0.38529] * u.au / u.s
...
def test_curtis53(lambert):
    k = Earth.k
    r0 = [273378.0, 0.0, 0.0] * u.au
    r = [145820.0, 12758.0, 0.0] * u.au
    tof = 13.5 * u.h
    numiter = 100

    # ERRATA: j component is positive
    expected_va = [-2.4356, 0.26741, 0.0] * u.au / u.s
...
def test_molniya_der_zero_full_revolution(lambert):
    k = Earth.k
    r0 = [22592.145603, -1599.915239, -19783.950506] * u.au
    r = [1922.067697, 4054.157051, -8925.727465] * u.au
    tof = 10 * u.h

    expected_va = [2.000652697, 0.387688615, -2.666947760] * u.au / u.s
    expected_vb = [-3.79246619, -1.77707641, 6.856814395] * u.au / u.s
...
def test_molniya_der_one_full_revolution(lambert):
    ...
    expected_va_l = [0.50335770, 0.61869408, -1.57176904] * u.au / u.s
    expected_vb_l = [-4.18334626, -1.13262727, 6.13307091] * u.au / u.s

    expected_va_r = [-2.45759553, 1.16945801, 0.43161258] * u.au / u.s
    expected_vb_r = [-5.53841370, 0.01822220, 5.49641054] * u.au / u.s
...
def test_collinear_vectors_input(lambert):
    k = Earth.k
    r0 = [22592.145603, -1599.915239, -19783.950506] * u.au
    r = [22592.145603, -1599.915239, -19783.950506] * u.au
    tof = 5 * u.h
...
def test_issue840(lambert_vallado, lambert_izzo):
    k = c.GM_earth.to(u.au ** 3 / u.s ** 2)
    r0 = [10000.0, 0, 0] * u.au
    rf = [8000.0, -5000, 0] * u.au
    tof = 2 * u.hour

    expected_va = [0.36591277, 5.8228806, 0.0] * u.au / u.s
    expected_vb = [3.99397599, 4.78236576, 0.0] * u.au / u.s
