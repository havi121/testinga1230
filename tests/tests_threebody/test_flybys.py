...
@pytest.mark.parametrize(
    ...
    [
        (0 * u.deg, [31.73, 1.766, 0] * u.au / u.s),
        (180 * u.deg, [37.14, -3.074, 0] * u.au / u.s),
    ],
)
def test_flyby_curtis(theta, expected_V_2_v):
    # Data from Curtis, Example 8.6
    periapsis_h = 300 * u.au
    V_1_v = [37.51, 2.782, 0] * u.au / u.s  # In some reference frame
    V = [35.02, 0, 0] * u.au / u.s

    expected_delta = 103.6 * u.deg

    V_2_v, delta = compute_flyby(V_1_v, V, Venus.k, Venus.R + periapsis_h, theta)

    assert_quantity_allclose(V_2_v, expected_V_2_v, rtol=1e-3, atol=1e-15 * u.au / u.s)
